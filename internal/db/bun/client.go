...
func myConn(ctx context.Context) (*Bun, error) {
	l := logger.WithField("func", "mysqlConn")
...
func deriveBunDBMyOptions() (string, error) {
	...
	if strings.ToUpper(viper.GetString(keys.DbType)) != db.TypeMysql {
		return "", fmt.Errorf("expected bun type of %s but got %s", db.TypePostgres, viper.GetString(keys.DbType))
	}
