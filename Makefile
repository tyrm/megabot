...
test-ext: tidy fmt lint #gosec
	MB_TLS_CERT=../../server.crt MB_TLS_KEY=../../server.key go test --tags=postgres,redis,mysql,mssql -cover ./...

test-bench-ext: tidy fmt lint #gosec
	MB_TLS_CERT=../../server.crt MB_TLS_KEY=../../server.key go test  -run=XXX -bench=. --tags=postgres,redis,mysql,mssql -cover ./...
...
test-race-ext: tidy fmt lint #gosec
	MB_TLS_CERT=../../server.crt MB_TLS_KEY=../../server.key go test --tags=postgres,redis,mysql,mssql -race -count=1 -cover ./...
